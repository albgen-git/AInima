...
INSERT INTO matching_algorithm_versions (versione, descrizione) VALUES
    ...
    ('stable_v9', 'Migrazione RF-08c/RF-11b da ArcFace/embedding precalcolato ad AWS Rekognition (v. CLAUDE.md): DetectFaces valida il volto all''upload (soglia 90%), CompareFaces confronta on-demand la foto "partner ideale" del cercatore con la foto profilo di ogni candidato in shortlist al momento della proposta — nulla di precalcolato/persistito, embedding_visivo_profilo/embedding_visivo_partner_ideale rimossi da physical_profile. Selezione visiva BIDIREZIONALE (entrambe le direzioni, non solo cercatore->candidato — intento di prodotto: l''abbinamento premia la coppia in cui ENTRAMBE le persone somigliano all''ideale dichiarato dall''altra), aggregata a MEDIA GEOMETRICA (non aritmetica) quando entrambe le direzioni sono calcolabili — la media aritmetica lasciava passare coppie con una direzione fortemente asimmetrica (es. 82/0.2 -> media 41), la geometrica collassa verso 0 in quel caso. Nessuna soglia minima di similarità (a differenza della calibrazione a percentile di stable_v5, pensata per un problema specifico di ArcFace che non si applica a un punteggio di confidenza "stessa persona" nativo come CompareFaces).')
