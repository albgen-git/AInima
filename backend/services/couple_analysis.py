...
def _profilo_psicometrico(u: dict) -> dict:
    """Solo i punteggi già aggregati di un lato della coppia (mai le
    risposte grezze agli item, mai flag/confidenze interne — stesso
    principio già applicato in services/personal_report.py)."""
    return {
        "big_five": {
        ...
        "attaccamento": {"ansia": u["ansia"], "evitamento": u["evitamento"]},
        "eq_maturita_emotiva": u["maturita"],
    }
...
def _assembla_punteggi_coppia(pool: dict, id_a: str, id_b: str, cfg: dict) -> dict:
    ...
    narrativa, _ = matching_engine.punteggio_narrativo_strutturato(a, b)
    soft, _ = matching_engine.combina_soft_e_distanza(a, b, punteggio_distanza)
