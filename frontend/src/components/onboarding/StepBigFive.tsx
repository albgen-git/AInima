...
import { BIG_FIVE_ITEMS } from "@/lib/wizard/bigFiveItems";
import type { StepProps } from "@/lib/wizard/types";

const SCALE = [1, 2, 3, 4, 5] as const;

export function StepBigFive({ state, update, onNext, onBack }: StepProps) {
  const t = useTranslations("onboarding.bigfive");
  ...
  const isLast = index === BIG_FIVE_ITEMS.length - 1;
      ...
      <div className="mt-6">
        <div className="h-1.5 w-full rounded-full bg-border">
          <div
            ...
            style={{ width: `${(index / BIG_FIVE_ITEMS.length) * 100}%` }}
          />
        </div>
        <p className="mt-2 text-xs text-slate">
          {t("progress", { current: index + 1, total: BIG_FIVE_ITEMS.length })}
        </p>
