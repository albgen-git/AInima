...
import { EQ_ITEMS } from "@/lib/wizard/eqItems";
import type { StepProps } from "@/lib/wizard/types";

const SCALE = [1, 2, 3, 4, 5] as const;

/**
 * Ainima_Test_EQScore_v1.md — sostituisce il rubric-scorer LLM sulla chat
 * con un test scritto a scoring deterministico (4 pilastri EQ), v.
 * CLAUDE.md 2026-08-19.
 */
export function StepEq({ state, update, onNext, onBack }: StepProps) {
  const t = useTranslations("onboarding.eq");
  const tCommon = useTranslations("common");
  const locale = useLocale();
  const [answers, setAnswers] = useState<Record<string, number>>({});
  const [index, setIndex] = useState(0);
  const { run, loading, error } = useAsyncAction(psychometricApi.submitEq);

  const item = EQ_ITEMS[index];
  const isLast = index === EQ_ITEMS.length - 1;
  ...
  async function selectAnswer(value: number) {
    ...
    if (result) {
      update("eqCompletato", true);
      onNext();
    }
  }

  if (state.eqCompletato) {
    return (
      <Card>
      ...
      <div className="mt-6">
        <div className="h-1.5 w-full rounded-full bg-border">
          <div
            ...
            style={{ width: `${(index / EQ_ITEMS.length) * 100}%` }}
          />
        </div>
        <p className="mt-2 text-xs text-slate">
          {t("progress", { current: index + 1, total: EQ_ITEMS.length })}
        </p>
